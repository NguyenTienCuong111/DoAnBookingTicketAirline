...
const {forgotPass} = require('../services/mailer')
...
const ForgotPass = async (req, res) => {
	...
	try {
	  ...
	  if (user) {
		...
		res.status(200).send({ message: "Send mail reset thành công" });
	  } else {
		res.status(404).send({ message: "Không tìm thấy tài khoản!" });
	  }
	} catch (error) {
	  console.error("Error in ForgotPass function:", error);
	  res.status(500).send({ message: "Lỗi trong quá trình xử lý yêu cầu" });
	}
  };
