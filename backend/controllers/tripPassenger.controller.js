...
const createTripPassenger = async (req, res) => {
	...
	try {
		...
		if (TripPassengerFind) {
			res.status(404).send("bay đã được phân vào chuyến khác vui lòng chọn bay hoặc Hãng bay khác");
		} else {
...
const getAllTripPassenger = async (req, res) => {
	const {tripId, start, from, to} = req.query;
	try {
		if (tripId) {
			const tripPassengerList = await TripPassenger.findAll({
				...
				include: [
					...
					// 	where: {
					// 		startTime: {
					// 			[Op.gte]: startTime + ' 00:00:00',
            		// 			[Op.lt]: startTime + ' 23:59:59'
					// 		}
					// 	}
					// },
					{
