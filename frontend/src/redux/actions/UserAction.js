...
export const ForgotAction = (user) => {
	return async (dispatch) => {
		try {
			const result = await userService.forgotService(user);
			console.log(result);
			if (result.status === 200) {
				dispatch({
					type: NOFICATION,
					title: "success",
					text: "Gửi thông báo thành công, vui lòng kiểm tra email.",
				});
...
export const ResetPassword = (user) => {
	return async (dispatch) => {
		try {
			const result = await userService.resetService(user);
