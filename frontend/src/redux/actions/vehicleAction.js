...
export const DeleteVehicleAction = (id) => {
	return async (dispatch) => {
		try {
			...
			if (result.status === 200) {
				message.success("Xóa bay thành công");
				dispatch(getAllVehicleAction());
			}
		} catch (error) {
			message.error("Xóa bay thất bại");
			console.log(error);
		}
	};
};
...
export const CreateImgVehicleAction = (id, file) => {
	return async (dispatch) => {
		try {
			...
			console.log(result);
			if (result.status === 200) {
				message.success("thêm ảnh bay thành công");
				dispatch(getAllVehicleAction());
			}
		} catch (error) {
			message.error("thêm ảnh bay thất bại");
			console.log(error);
		}
	};
};
...
export const deleteImgVehicleAction = (id) => {
	return async (dispatch) => {
		try {
			...
			if (result.status === 200) {
				dispatch(getAllVehicleAction());
				message.success("Xóa ảnh bay thành công");
			}
		} catch (error) {
			message.error("Xóa ảnh bay thất bại");
			console.log(error);
		}
	};
};
