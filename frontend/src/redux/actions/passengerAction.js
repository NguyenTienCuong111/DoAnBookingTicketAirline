...
export const updatePassengerAction = (passenger, id, file) => {
	return async (dispatch) => {
		try {
			...
			if (result.status == 200) {
				message.success("update  bay thành công");
				dispatch(getAllPassenger());
			}
		} catch (error) {
			message.error("update  bay thất bại");
			console.log(error);
		}
	};
};
...
export const updateImagePassengerAction = (id, file) => {
	return async (dispatch) => {
		try {
			...
			if (result.status == 200) {
				message.success("update images bay thành công");
				dispatch(getAllPassenger());
			}
		} catch (error) {
			message.error("update images bay thất bại");
			console.log(error);
		}
	};
};
...
export const deletePassenger = (id) => {
	return async (dispatch) => {
		try {
			...
			if (result.status == 200) {
				message.success("delete  passenger bay thành công");
				dispatch(getAllPassenger());
			}
		} catch (error) {
			message.error("elete  passenger bay  thất bại");
			console.log(error);
		}
	};
};
	...
	return async (dispatch) => {
		try {
			...
			if (result.status == 200) {
				...
				message.success("thêm  passenger bay thành công");
				dispatch(getAllPassenger());
			}
		} catch (error) {
			message.error("thêm  passenger bay  thất bại");
			console.log(error);
		}
	};
};
