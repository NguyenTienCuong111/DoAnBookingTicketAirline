	...
	return async (dispatch) => {
		try {
			...
			if (result.status == 200) {
				message.success("thêm ghế bay thành công");
				dispatch(getAllVehicleAction());
				dispatch(getDetailVehicleAction(id));
			}
		} catch (error) {
			message.error("thêm ghế bay thất bại");
			console.log(error);
		}
	};
};
...
export const UpdateSeatAction = (id, seat, vehicleId) => {
	return async (dispatch) => {
		try {
			...
			console.log(result);
			if (result.status == 200) {
				message.success("cập nhật ghế bay thành công");
				dispatch(getAllVehicleAction());
				dispatch(getDetailVehicleAction(vehicleId));
			}
		} catch (error) {
			message.error("cập nhật ghế bay thất bại");
			console.log(error);
		}
	};
};
...
export const DeleteSeatAction = (id, vehicleId) => {
	return async (dispatch) => {
		try {
			...
			console.log(result);
			if (result.status == 200) {
				message.success("Xóa ghế bay thành công");
				dispatch(getAllVehicleAction());
				dispatch(getDetailVehicleAction(vehicleId));
			}
		} catch (error) {
			message.error("Xóa ghế bay thất bại");
			console.log(error);
		}
	};
};
