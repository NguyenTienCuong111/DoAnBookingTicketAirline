	...
	return (
		<div>
			<Descriptions title="Thông tin vé" layout="vertical" bordered labelStyle={{fontWeight: "bold", fontSize: 16}} contentStyle={{fontSize: 12}}>
				...
				<Descriptions.Item label="Hãng bay">{ticket.tripPassengerTicket.passenger.name}</Descriptions.Item>
				<Descriptions.Item label="Tên bay">{ticket.tripPassengerTicket.vehicle.name}</Descriptions.Item>
				<Descriptions.Item label="Hình ảnh bay">
					<div>
				...
				<Descriptions.Item label="Kết thúc (dự kiến)">
					{moment(ticket.tripPassengerTicket.trip?.startTime).format("DD-MM-YYYY")} {ticket.tripPassengerTicket.endTime}
				</Descriptions.Item>
				...
				<Descriptions.Item label="Lưu ý">Mọi thủ tục vui lòng đến Hãng bay thông tin trên vé . Hãng bay không chịu trách nhiệm</Descriptions.Item>
			</Descriptions>
			,
		</div>
	);
