	...
	const columns = [
		...
		},
		{
			...
			render: (text, passenger) => {
				return (
					<div>
						<Image width={150} src={passenger.imageIntro} style={{objectFit:'cover'}} />
					</div>
				);
			},
		},
		{
			title: "Số Máy Bay Sở Hữu",
			render: (text, passenger) => {
				return (
					<div>
						<Statistic title="Số hiệu" value={passenger.passengerCar.length} />
						<Button
							type="primary"
							onClick={() => {
								dispatch({
									type: SET_MODAL,
									title: `Danh sách máy bay của hãng ${passenger.name}`,
				...
				return (
					<Fragment>
						<div>
							...
							<Popconfirm
								placement="topLeft"
								title={"Bạn có muốn xóa hãng bay này"}
								onConfirm={() => {
